...
class Trainer:
    def __init__(self, model, criterion, optimizer,
        ...
        # Copy the content of the encoder of labels for reuse
        self.label_encoder = label_encoder
        np.save(self.save_dir / 'le_classes.npy', self.label_encoder.classes_)
    ...
    def _train_epoch(self, epoch):
        ...
        for idx, (batch_sequences, batch_labels) in enumerate(self.train_dataloader):
            ...
            predicted_labels = self.model(batch_sequences)

            # Compute the loss and backpropagate
            loss = self.criterion(predicted_labels, batch_labels)
            loss.backward()
            # Clip the gradients (it rescales to the specified norm
            # if the norm of gradients exceeds the set value)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for idx, (batch_sequences, batch_labels) in enumerate(self.valid_dataloader):
                # Transfer to data to device (necessary for GPU)
                batch_sequences = batch_sequences.to(self.device)
                batch_labels = batch_labels.to(self.device)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for _, (batch_sequences, batch_labels) in enumerate(self.test_dataloader):
                # Transfer to data to device (necessary for GPU)
                batch_sequences = batch_sequences.to(self.device)
                batch_labels = batch_labels.to(self.device)
